...
// Fetches a list of contacts associated to a particular platform
export const fetchPlatformContacts = (id: any) => (dispatch: any) => {
  dispatch(actions.startCall(any))
      axios.get(`${PLATFORMS_URL}/${id}/platform_contacts`, optionsHeaders())
    //   axios.get(`${PLATFORMS_URL}/platform_contacts/${id}`, optionsHeaders())
      .then(function (response) {
          console.log(id)
          console.log(response.data)
          return dispatch(actions.platformContactsReceived(response.data));
      })
      .catch(function (error) {
          return dispatch(actions.catchError(error));
      });
